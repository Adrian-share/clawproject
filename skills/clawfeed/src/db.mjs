...
export function getDb(dbPath) {
  ...
  // Migration 010: signal items storage (Reddit/local ingestion)
  try {
    const sql10 = readFileSync(join(ROOT, 'migrations', '010_signal_items.sql'), 'utf8');
    _db.exec(sql10);
  } catch (e) {
...
// ── Signal Items (local ingest storage) ──

export function upsertSignalItems(db, items = []) {
  ...
  const run = db.transaction((rows) => {
    ...
    for (const row of rows) {
      const before = db.prepare('SELECT id FROM signal_items WHERE source = ? AND external_id = ?').get(row.source, row.external_id);
      stmt.run(row);
      if (before) updated += 1;
      else inserted += 1;
    }
...
export function listSignalItems(db, { since, limit = 200, source } = {}) {
  let sql = 'SELECT * FROM signal_items';
  const cond = [];
  const params = [];
  if (since) { cond.push('published_at >= ?'); params.push(since); }
