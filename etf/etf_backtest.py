...
class BacktestEngine:
    def __init__(self, buy_amount=100):
        ...
        self.cash = 0  # 初始现金
        self.holdings = {}  # 持仓 {code: {'shares': 0, 'avg_price': 0}}
        self.trades = []  # 交易记录
    ...
    def calculate_signals(self, prices):
        ...
        # 买入信号: M5 > M10 > M20 且每个均线都在上升
        ma_alignment = (m5 > m10) and (m10 > m20)
        ma_rising = True
        if len(prices) >= 6:
            m5_prev = sum(prices[-6:-1]) / 5
            m10_prev = sum(prices[-11:-1]) / 10
    ...
    def run_backtest(self, df, code, name):
        ...
        df = df.tail(252)  # 近一年的数据（约252个交易日）
        ...
        trades = []
        daily_values = []
        buy_signal_active = False  # 标记买入信号是否持续
        
        for i in range(20, len(df)):  # 从第20天开始（需要足够历史计算均线）
            row = df.iloc[i]
            date = row['date']
            price = row['close']
            ...
            # 计算信号
            buy_signal, sell_signal = self.calculate_signals(prices)
        ...
        return {
            ...
            'total_profit': final_value,  # 净收益（投入现金+持仓价值）
            'profit_pct': (final_value / self.buy_amount - 1) * 100 if self.buy_amount > 0 else 0,
            'holdings': holdings,
    ...
    if result['trades']:
        ...
        for trade in trades_to_show:
            if trade['action'] == 'BUY_START':
                report_lines.append(f"  {trade['date']} 🟢 买入信号出现")
            elif trade['action'] == 'BUY_DAILY':
                report_lines.append(f"  {trade['date']} 💰 买入 {trade['amount']:.2f} 元 @ {trade['price']:.4f}")
            elif trade['action'] == 'SELL_ALL':
                report_lines.append(f"  {trade['date']} 🔴 卖出全部 @ {trade['price']:.4f}, 回收 {trade['amount']:.2f} 元")
        
        if len(result['trades']) > 10:
            report_lines.append(f"  ... 还有 {len(result['trades']) - 10} 笔交易")
...
# 保存报告
report_file = REPORT_DIR / ("backtest_" + datetime.now().strftime('%Y%m%d_%H%M%S') + ".txt")
...
def jsonify_result(result):
    ...
    # 转换交易记录中的日期
    result_copy['trades'] = [
        {**t, 'date': str(t['date'])} for t in result_copy['trades']
    ]
    
    # 转换每日价值记录中的日期
    result_copy['daily_values'] = [
        {**d, 'date': str(d['date'])} for d in result_copy['daily_values']
    ]
    
    return result_copy

json_file = REPORT_DIR / ("backtest_" + datetime.now().strftime('%Y%m%d_%H%M%S') + ".json")
