#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ETF 趋势监控 Agent - 完整状态模式
显示所有当前满足条件的ETF，不只显示变化
"""
...
for etf in etf_list:
    ...
    try:
        ...
        analysis = {
            ...
            'buy_signal': buy_signal,  # 直接使用当前状态
            'sell_signal': sell_signal,  # 直接使用当前状态
...
report_lines.extend([
    ...
    "当前买入信号: %d 个" % len(results['buy_signals']),
    "当前卖出信号: %d 个" % len(results['sell_signals']),
...
# 保存报告
report_file = REPORT_DIR / ("report_full_" + datetime.now().strftime('%Y%m%d') + ".txt")
